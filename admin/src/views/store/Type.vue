    ...
    <n-drawer v-model:show="form.activeDrawer" width="50%" :close-on-esc="false" :mask-closable="false">
      <n-drawer-content :title="form.title">
        <n-form ref="formRef" :model="form" label-placement="left" label-width="auto"
          ...
          <n-form-item label="是否平台类型">
            <n-radio-group v-model:value="form.is_only_sys">
...
import { getStoreBase, postStoreType, deleteStoreType } from '@/api/store';
  ...
  },
  {
    ...
    render(row) {
      return h(
        NTag,
        {
          type: row.is_only_sys ? 'success' : 'error',
        },
        {
          default: () => (row.is_only_sys ? '是' : '否'),
        }
      );
    },
  },
...
const handleEdit = (record: Recordable) => {
  ...
  form.is_only_sys = record.is_only_sys ? 1 : 0
...
const handleDelete = (record: Recordable) => {
  ...
  dialog.warning({
    ...
    negativeText: '取消',
    onPositiveClick: () => {
      deleteStoreType(record.id).then(() => init())
    },
  })
}
// 提交表单
  ...
  formRef.value?.validate((errors) => {
    if (!errors) {
      ...
      const formData = {
        ...
        is_only_sys: form.is_only_sys,
        type_order: form.type_order,
      }

      form.loading = true

      postStoreType(formData).then(() => {
        resetData()
        init();
      }).finally(() => {
        form.loading = false
