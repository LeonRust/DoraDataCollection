      ...
      <n-drawer-content title="添加task">
        <n-form ref="formRef" :model="form" label-placement="left" label-width="auto"
          ...
          <n-form-item label="标签">
            <n-input v-model:value="form.tag" placeholder="请输入机器人标签" />
          </n-form-item>
        </n-form>
...
import { createTask, getTaskList } from '@/api/dashboard/workplace';
...
const submitForm = (e: MouseEvent) => {
  ...
  formRef.value?.validate((errors) => {
    if (!errors) {
      ...
      createTask({name: form.name, info: form.info }).then(() => {
        init();
        resetData();
      })
      .catch(() => {
